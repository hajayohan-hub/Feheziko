...
function put(word, type, translation, definition, example_fr, example_mg) {
  if (!word || !translation) return;
  const key = word.toLowerCase().trim();
  if (wordsMap.has(key)) return;
  wordsMap.set(key, {
    word: word.trim(),
    type: type.trim(),
...
// 1. Initial fr.json items
const frJson = JSON.parse(fs.readFileSync('./src/languages/fr.json', 'utf8'));
(frJson.dictionary || []).forEach(item => {
  put(item.word, item.type, item.translation, item.definition, item.example_fr, item.example_mg);
});

// 2. Additional entries categorized cleanly
const entries = [
  // Block 1: Expressions & Salutations
...
entries.forEach(r => put(...r));

console.log("Current entries:", wordsMap.size);
